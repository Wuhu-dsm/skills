...
### Paragraph `attr` Keys
`jc` (alignment), `class` (style), `list`/`list-level`, `ind-left`/`ind-right`/`ind-first`/`ind-hanging`, `spacing-before`/`spacing-after`, `line-spacing`, `spacing-line-rule`, `keep-next`, `page-break-before`, `shd`, `outline-level`, `border-*`, `bidi`
...
1. **Style-default sweep via `part="styles"`.** Read the styles view, then edit each affected `<style id="...">` entry with `TextEdit`s. Changes propagate to every paragraph that does not carry an inline override. `patch_styles` accepts these keys on `<style>`: `font-size`, `ascii`, `hAnsi`, `eastAsia`, `line-spacing`, `lineRule`, `firstLine`, `hanging`, `spacing-before`, `spacing-after`, `jc`, `b`, `i`, `color`, `page-break-before`, plus the usual meta (`name`, `basedOn`, `next`, `uiPriority`, `semiHidden`, `unhideWhenUsed`, `qFormat`, `outline`, `default`, `custom`).
